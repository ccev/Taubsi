...
class StyleSelect(discord.ui.Select):
    def __init__(self, dmap: Map, settings: Settings):
        ...
        for i, style in enumerate(bot.config.DMAP_STYLES):
            self.options.append(discord.SelectOption(label=style.name, value=style.id, default=i == 0))
        self.dmap = dmap
...
class IconSelect(discord.ui.Select):
    def __init__(self, dmap: Map, settings: Settings):
        ...
        for i, iconset in enumerate(IconSet):
            self.options.append(discord.SelectOption(label=iconset.value.name, value=iconset.name, default=i == 0))
        self.dmap = dmap
        self.settings = settings
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.dmap.iconset = IconSet[iconset_name]
        for option in self.options:
...
class Settings(discord.ui.View):
    ...
    def make_embed(self):
        text = (
            f"Map Style: **{self.dmap.style.name}**\n"
            f"Map size: **{self.dmap.width}x{self.dmap.height}px**\n"
            f"Icons: **{self.dmap.iconset.value.name}**\n"
